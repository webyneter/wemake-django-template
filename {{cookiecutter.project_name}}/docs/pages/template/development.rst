...
If you are using ``docker`` then prepend ``./run development run --rm web``
...
.. code:: bash
  ...
  ./run development run --rm web python manage.py migrate
  ./run development up
...
we use ``./run development run`` to run scripts inside docker.
